...
#align(center)[
  #cetz.canvas({
    ...
    // ========================================================================
    // Аккумулятор (увеличен влево для размножения знака)
    rect((-5, y_acc_b), (17, y_acc_t), name: "acc", stroke: 1pt, fill: white)
    line((-3.5, y_acc_b), (-3.5, y_acc_t), stroke: 1pt) // Отделение знакового разряда
    content((-4.25, (y_acc_b + y_acc_t)/2), [ЗН])
    content((6.5, (y_acc_b + y_acc_t)/2), [*АККУМУЛЯТОР*])

    // Стрелка общего сдвига аккумулятора (Алгоритм В - сдвиг влево)
    wire((11, y_acc_t + 0.5), (1, y_acc_t + 0.5))

    // ФДК (Сделан короче, до X=17)
    rect((0, y_fdk_b), (17, y_fdk_t), name: "fdk", stroke: 1pt, fill: white)
    line((1.5, y_fdk_b), (1.5, y_fdk_t), stroke: 1pt)
    content((0.75, (y_fdk_b + y_fdk_t)/2), [ЗН])
    content((8.5, (y_fdk_b + y_fdk_t)/2), [*ФДК*])

    // Регистр множимого (Сделан короче, до X=17)
    rect((0, y_rmg_b), (17, y_rmg_t), name: "rmg", stroke: 1pt, fill: white)
    line((1.5, y_rmg_b), (1.5, y_rmg_t), stroke: 1pt)
    content((0.75, (y_rmg_b + y_rmg_t)/2), [ЗН])
    content((8.5, (y_rmg_b + y_rmg_t)/2), [*РЕГИСТР МНОЖИМОГО*])
    ...
    wire((22.5, y_q1), (24.5, y_q1)); content((23.5, y_q1 + 0.3), [$Q_n$])
    wire((22.5, y_q2), (24.5, y_q2)); content((23.5, y_q2 + 0.3), [$Q_(n-1)$])
    wire((22.5, y_q3), (24.5, y_q3)); content((23.5, y_q3 + 0.3), [$Q_(n-2)$])
    ...
    dotbig((28.3, y_q4))
    bus_stealth((28.3, y_q3), (28.3, y_q3), (28.3, y_ctrl), (3.5, y_ctrl))

    // Сигнал Mul/Sum (h) извне справа
    wire_stealth((30.5, y_h), (3.0, y_h)) // Линия заканчивается на X=3.0 (последний ОЧУ)
    ...
    let left_cx = 15.5 - 4 * sp // = 3.5
    wire((left_cx - ocs_w/2, (y_ocs_b + y_ocs_t)/2), (2, (y_ocs_b + y_ocs_t)/2), (2, y_acc_b))
    ...
    bus((4, y_rmg_b - 0.7), (4, y_rmg_b));       content((4, y_rmg_b - 1), [$D_m$])
    bus((2.5, y_rmg_b - 0.7), (2.5, y_rmg_b));   content((2.5, y_rmg_b - 1), [$D_(m+1)$])
    bus((0.8, y_rmg_b - 0.7), (0.8, y_rmg_b));   content((0.8, y_rmg_b - 1), [$D_(m+2)$])
    ...
    wire((0.75, y_rmg_t), (0.75, y_fdk_b))

    // РАЗМНОЖЕНИЕ ЗНАКА (от ФДК в аккумулятор)
    let y_sign_dist = 10.4 // Уровень горизонтальной шины знака
    line((0.75, y_fdk_t), (0.75, y_sign_dist))
    line((0.75, y_sign_dist), (-4.25, y_sign_dist))
    dot((0.75, y_sign_dist))

    wire((-4.25, y_sign_dist), (-4.25, y_acc_b)) // В дальний ЗН аккумулятора
    wire((-1.75, y_sign_dist), (-1.75, y_acc_b)) // В промежуточный доп. разряд (размноженный знак)
    wire((0.75, y_sign_dist), (0.75, y_acc_b))   // Оригинальный вход ЗН

    content((-0.5, y_acc_b - 0.3), [$. . .$]) // Многоточие для показа размножения
  })
]